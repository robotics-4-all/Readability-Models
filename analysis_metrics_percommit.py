...
for metric in ['q1a_ttest', 'q1b_ttest', 'q1a_wilcoxon', 'q1b_wilcoxon']:
	results1[metric + '_stat'] = results1[metric].map(lambda x: x.statistic, na_action='ignore')
	results1[metric + '_pval'] = results1[metric].map(lambda x: x.pvalue, na_action='ignore')
# unpack the statistic values into plain numeric columns

results1.to_csv('results_Q1_percommit2.csv')
...
results2.to_csv('results_Q2_percommit2.csv')
